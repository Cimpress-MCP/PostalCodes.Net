...
{
    [TestFixture]
    internal class PEPostalCodeTests
    {
        ...
        {
            var code = new PEPostalCode(postalCode);
            var codePredecessor = new PEPostalCode(postalCodePredecessor);
        ...
        {
            var code = new PEPostalCode(postalCode);
            var codeSuccessor = new PEPostalCode(postalCodeSuccessor);
        ...
        {
            Assert.IsNull((new PEPostalCode(postalCode)).Predecessor);
        }

        [TestCase("99999")]
        public void Successor_LastInRange_ReturnsNull(string postalCode)
        {
            Assert.IsNull((new PEPostalCode(postalCode)).Successor);
        }
        ...
        {
            Assert.Throws<ArgumentException>(() => new PEPostalCode(postalCode));
        }

        public void CompareTo_ReturnsExpectedSign(string postalCodeBefore, string postalCodeAfter)
        {
            var b = new PEPostalCode(postalCodeBefore);
            var a = new PEPostalCode(postalCodeAfter);
        ...
        {
            var x = (new PEPostalCode(code)).Predecessor;
            bool result = true;
        ...
        {
            var x = (new PEPostalCode(code)).Predecessor;
            Assert.IsTrue(x.GetType() == typeof(PEPostalCode));
        }
        ...
        {
            var x = (new PEPostalCode(code)).Successor;
            Assert.IsTrue(x.GetType() == typeof(PEPostalCode));
        }
        ...
        {
            var x = (new PEPostalCode(code)).ExpandPostalCodeAsHighestInRange();
            Assert.IsTrue(x.GetType() == typeof(PEPostalCode));
        }
        ...
        {
            var x = (new PEPostalCode(code)).ExpandPostalCodeAsLowestInRange();
            Assert.IsTrue(x.GetType() == typeof(PEPostalCode));
        }
        ...
        {
            var x = new PEPostalCode(code);
            var y = new PEPostalCode(code);
        ...
        {
            var country = CountryFactory.Instance.CreateCountry("PE");
