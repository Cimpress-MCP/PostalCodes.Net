...
{
    [TestFixture]
    internal class HRPostalCodeTests
    {
        ...
        {
            var code = new HRPostalCode(postalCode);
            var codePredecessor = new HRPostalCode(postalCodePredecessor);
        ...
        {
            var code = new HRPostalCode(postalCode);
            var codeSuccessor = new HRPostalCode(postalCodeSuccessor);
        ...
        {
            Assert.IsNull((new HRPostalCode(postalCode)).Predecessor);
        }

        [TestCase("99999")]
        public void Successor_LastInRange_ReturnsNull(string postalCode)
        {
            Assert.IsNull((new HRPostalCode(postalCode)).Successor);
        }
        ...
        {
            Assert.Throws<ArgumentException>(() => new HRPostalCode(postalCode));
        }

        public void CompareTo_ReturnsExpectedSign(string postalCodeBefore, string postalCodeAfter)
        {
            var b = new HRPostalCode(postalCodeBefore);
            var a = new HRPostalCode(postalCodeAfter);
        ...
        {
            var x = (new HRPostalCode(code)).Predecessor;
            bool result = true;
        ...
        {
            var x = (new HRPostalCode(code)).Predecessor;
            Assert.IsTrue(x.GetType() == typeof(HRPostalCode));
        }
        ...
        {
            var x = (new HRPostalCode(code)).Successor;
            Assert.IsTrue(x.GetType() == typeof(HRPostalCode));
        }
        ...
        {
            var x = (new HRPostalCode(code)).ExpandPostalCodeAsHighestInRange();
            Assert.IsTrue(x.GetType() == typeof(HRPostalCode));
        }
        ...
        {
            var x = (new HRPostalCode(code)).ExpandPostalCodeAsLowestInRange();
            Assert.IsTrue(x.GetType() == typeof(HRPostalCode));
        }
        ...
        {
            var x = new HRPostalCode(code);
            var y = new HRPostalCode(code);
        ...
        {
            var country = CountryFactory.Instance.CreateCountry("HR");
