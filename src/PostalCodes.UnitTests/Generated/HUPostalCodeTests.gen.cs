...
{
    [TestFixture]
    internal class HUPostalCodeTests
    {
        ...
        {
            var code = new HUPostalCode(postalCode);
            var codePredecessor = new HUPostalCode(postalCodePredecessor);
        ...
        {
            var code = new HUPostalCode(postalCode);
            var codeSuccessor = new HUPostalCode(postalCodeSuccessor);
        ...
        {
            Assert.IsNull((new HUPostalCode(postalCode)).Predecessor);
        }

        [TestCase("9999")]
        public void Successor_LastInRange_ReturnsNull(string postalCode)
        {
            Assert.IsNull((new HUPostalCode(postalCode)).Successor);
        }
        ...
        {
            Assert.Throws<ArgumentException>(() => new HUPostalCode(postalCode));
        }

        public void CompareTo_ReturnsExpectedSign(string postalCodeBefore, string postalCodeAfter)
        {
            var b = new HUPostalCode(postalCodeBefore);
            var a = new HUPostalCode(postalCodeAfter);
        ...
        {
            var x = (new HUPostalCode(code)).Predecessor;
            bool result = true;
        ...
        {
            var x = (new HUPostalCode(code)).Predecessor;
            Assert.IsTrue(x.GetType() == typeof(HUPostalCode));
        }
        ...
        {
            var x = (new HUPostalCode(code)).Successor;
            Assert.IsTrue(x.GetType() == typeof(HUPostalCode));
        }
        ...
        {
            var x = (new HUPostalCode(code)).ExpandPostalCodeAsHighestInRange();
            Assert.IsTrue(x.GetType() == typeof(HUPostalCode));
        }
        ...
        {
            var x = (new HUPostalCode(code)).ExpandPostalCodeAsLowestInRange();
            Assert.IsTrue(x.GetType() == typeof(HUPostalCode));
        }
        ...
        {
            var x = new HUPostalCode(code);
            var y = new HUPostalCode(code);
        ...
        {
            var country = CountryFactory.Instance.CreateCountry("HU");
