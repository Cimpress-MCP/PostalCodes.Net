...
{
    [TestFixture]
    internal class FIPostalCodeTests
    {
        ...
        {
            var code = new FIPostalCode(postalCode);
            var codePredecessor = new FIPostalCode(postalCodePredecessor);
        ...
        {
            var code = new FIPostalCode(postalCode);
            var codeSuccessor = new FIPostalCode(postalCodeSuccessor);
        ...
        {
            Assert.IsNull((new FIPostalCode(postalCode)).Predecessor);
        }

        [TestCase("99999")]
        public void Successor_LastInRange_ReturnsNull(string postalCode)
        {
            Assert.IsNull((new FIPostalCode(postalCode)).Successor);
        }
        ...
        {
            Assert.Throws<ArgumentException>(() => new FIPostalCode(postalCode));
        }

        public void CompareTo_ReturnsExpectedSign(string postalCodeBefore, string postalCodeAfter)
        {
            var b = new FIPostalCode(postalCodeBefore);
            var a = new FIPostalCode(postalCodeAfter);
        ...
        {
            var x = (new FIPostalCode(code)).Predecessor;
            bool result = true;
        ...
        {
            var x = (new FIPostalCode(code)).Predecessor;
            Assert.IsTrue(x.GetType() == typeof(FIPostalCode));
        }
        ...
        {
            var x = (new FIPostalCode(code)).Successor;
            Assert.IsTrue(x.GetType() == typeof(FIPostalCode));
        }
        ...
        {
            var x = (new FIPostalCode(code)).ExpandPostalCodeAsHighestInRange();
            Assert.IsTrue(x.GetType() == typeof(FIPostalCode));
        }
        ...
        {
            var x = (new FIPostalCode(code)).ExpandPostalCodeAsLowestInRange();
            Assert.IsTrue(x.GetType() == typeof(FIPostalCode));
        }
        ...
        {
            var x = new FIPostalCode(code);
            var y = new FIPostalCode(code);
        ...
        {
            var country = CountryFactory.Instance.CreateCountry("FI");
