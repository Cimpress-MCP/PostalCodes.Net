...
{
    [TestFixture]
    internal class NOPostalCodeTests
    {
        ...
        {
            var code = new NOPostalCode(postalCode);
            var codePredecessor = new NOPostalCode(postalCodePredecessor);
        ...
        {
            var code = new NOPostalCode(postalCode);
            var codeSuccessor = new NOPostalCode(postalCodeSuccessor);
        ...
        {
            Assert.IsNull((new NOPostalCode(postalCode)).Predecessor);
        }

        [TestCase("9999")]
        public void Successor_LastInRange_ReturnsNull(string postalCode)
        {
            Assert.IsNull((new NOPostalCode(postalCode)).Successor);
        }
        ...
        {
            Assert.Throws<ArgumentException>(() => new NOPostalCode(postalCode));
        }

        public void CompareTo_ReturnsExpectedSign(string postalCodeBefore, string postalCodeAfter)
        {
            var b = new NOPostalCode(postalCodeBefore);
            var a = new NOPostalCode(postalCodeAfter);
        ...
        {
            var x = (new NOPostalCode(code)).Predecessor;
            bool result = true;
        ...
        {
            var x = (new NOPostalCode(code)).Predecessor;
            Assert.IsTrue(x.GetType() == typeof(NOPostalCode));
        }
        ...
        {
            var x = (new NOPostalCode(code)).Successor;
            Assert.IsTrue(x.GetType() == typeof(NOPostalCode));
        }
        ...
        {
            var x = (new NOPostalCode(code)).ExpandPostalCodeAsHighestInRange();
            Assert.IsTrue(x.GetType() == typeof(NOPostalCode));
        }
        ...
        {
            var x = (new NOPostalCode(code)).ExpandPostalCodeAsLowestInRange();
            Assert.IsTrue(x.GetType() == typeof(NOPostalCode));
        }
        ...
        {
            var x = new NOPostalCode(code);
            var y = new NOPostalCode(code);
        ...
        {
            var country = CountryFactory.Instance.CreateCountry("NO");
