...
{
    [TestFixture]
    internal class CRPostalCodeTests
    {
        ...
        {
            var code = new CRPostalCode(postalCode);
            var codePredecessor = new CRPostalCode(postalCodePredecessor);
        ...
        {
            var code = new CRPostalCode(postalCode);
            var codeSuccessor = new CRPostalCode(postalCodeSuccessor);
        ...
        {
            Assert.IsNull((new CRPostalCode(postalCode)).Predecessor);
        }

        [TestCase("99999")]
        public void Successor_LastInRange_ReturnsNull(string postalCode)
        {
            Assert.IsNull((new CRPostalCode(postalCode)).Successor);
        }
        ...
        {
            Assert.Throws<ArgumentException>(() => new CRPostalCode(postalCode));
        }

        public void CompareTo_ReturnsExpectedSign(string postalCodeBefore, string postalCodeAfter)
        {
            var b = new CRPostalCode(postalCodeBefore);
            var a = new CRPostalCode(postalCodeAfter);
        ...
        {
            var x = (new CRPostalCode(code)).Predecessor;
            bool result = true;
        ...
        {
            var x = (new CRPostalCode(code)).Predecessor;
            Assert.IsTrue(x.GetType() == typeof(CRPostalCode));
        }
        ...
        {
            var x = (new CRPostalCode(code)).Successor;
            Assert.IsTrue(x.GetType() == typeof(CRPostalCode));
        }
        ...
        {
            var x = (new CRPostalCode(code)).ExpandPostalCodeAsHighestInRange();
            Assert.IsTrue(x.GetType() == typeof(CRPostalCode));
        }
        ...
        {
            var x = (new CRPostalCode(code)).ExpandPostalCodeAsLowestInRange();
            Assert.IsTrue(x.GetType() == typeof(CRPostalCode));
        }
        ...
        {
            var x = new CRPostalCode(code);
            var y = new CRPostalCode(code);
        ...
        {
            var country = CountryFactory.Instance.CreateCountry("CR");
