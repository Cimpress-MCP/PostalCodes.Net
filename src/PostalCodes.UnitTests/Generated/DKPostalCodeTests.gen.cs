...
{
    [TestFixture]
    internal class DKPostalCodeTests
    {
        ...
        {
            var code = new DKPostalCode(postalCode);
            var codePredecessor = new DKPostalCode(postalCodePredecessor);
        ...
        {
            var code = new DKPostalCode(postalCode);
            var codeSuccessor = new DKPostalCode(postalCodeSuccessor);
        ...
        {
            Assert.IsNull((new DKPostalCode(postalCode)).Predecessor);
        }

        [TestCase("9999")]
        public void Successor_LastInRange_ReturnsNull(string postalCode)
        {
            Assert.IsNull((new DKPostalCode(postalCode)).Successor);
        }
        ...
        {
            Assert.Throws<ArgumentException>(() => new DKPostalCode(postalCode));
        }

        public void CompareTo_ReturnsExpectedSign(string postalCodeBefore, string postalCodeAfter)
        {
            var b = new DKPostalCode(postalCodeBefore);
            var a = new DKPostalCode(postalCodeAfter);
        ...
        {
            var x = (new DKPostalCode(code)).Predecessor;
            bool result = true;
        ...
        {
            var x = (new DKPostalCode(code)).Predecessor;
            Assert.IsTrue(x.GetType() == typeof(DKPostalCode));
        }
        ...
        {
            var x = (new DKPostalCode(code)).Successor;
            Assert.IsTrue(x.GetType() == typeof(DKPostalCode));
        }
        ...
        {
            var x = (new DKPostalCode(code)).ExpandPostalCodeAsHighestInRange();
            Assert.IsTrue(x.GetType() == typeof(DKPostalCode));
        }
        ...
        {
            var x = (new DKPostalCode(code)).ExpandPostalCodeAsLowestInRange();
            Assert.IsTrue(x.GetType() == typeof(DKPostalCode));
        }
        ...
        {
            var x = new DKPostalCode(code);
            var y = new DKPostalCode(code);
        ...
        {
            var country = CountryFactory.Instance.CreateCountry("DK");
