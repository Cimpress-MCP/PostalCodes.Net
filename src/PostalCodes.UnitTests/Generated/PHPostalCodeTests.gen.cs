...
{
    [TestFixture]
    internal class PHPostalCodeTests
    {
        ...
        {
            var code = new PHPostalCode(postalCode);
            var codePredecessor = new PHPostalCode(postalCodePredecessor);
        ...
        {
            var code = new PHPostalCode(postalCode);
            var codeSuccessor = new PHPostalCode(postalCodeSuccessor);
        ...
        {
            Assert.IsNull((new PHPostalCode(postalCode)).Predecessor);
        }

        [TestCase("9999")]
        public void Successor_LastInRange_ReturnsNull(string postalCode)
        {
            Assert.IsNull((new PHPostalCode(postalCode)).Successor);
        }
        ...
        {
            Assert.Throws<ArgumentException>(() => new PHPostalCode(postalCode));
        }

        public void CompareTo_ReturnsExpectedSign(string postalCodeBefore, string postalCodeAfter)
        {
            var b = new PHPostalCode(postalCodeBefore);
            var a = new PHPostalCode(postalCodeAfter);
        ...
        {
            var x = (new PHPostalCode(code)).Predecessor;
            bool result = true;
        ...
        {
            var x = (new PHPostalCode(code)).Predecessor;
            Assert.IsTrue(x.GetType() == typeof(PHPostalCode));
        }
        ...
        {
            var x = (new PHPostalCode(code)).Successor;
            Assert.IsTrue(x.GetType() == typeof(PHPostalCode));
        }
        ...
        {
            var x = (new PHPostalCode(code)).ExpandPostalCodeAsHighestInRange();
            Assert.IsTrue(x.GetType() == typeof(PHPostalCode));
        }
        ...
        {
            var x = (new PHPostalCode(code)).ExpandPostalCodeAsLowestInRange();
            Assert.IsTrue(x.GetType() == typeof(PHPostalCode));
        }
        ...
        {
            var x = new PHPostalCode(code);
            var y = new PHPostalCode(code);
        ...
        {
            var country = CountryFactory.Instance.CreateCountry("PH");
