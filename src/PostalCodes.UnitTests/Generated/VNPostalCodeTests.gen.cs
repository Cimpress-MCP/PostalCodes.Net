...
{
    [TestFixture]
    internal class VNPostalCodeTests
    {
        ...
        {
            var code = new VNPostalCode(postalCode);
            var codePredecessor = new VNPostalCode(postalCodePredecessor);
        ...
        {
            var code = new VNPostalCode(postalCode);
            var codeSuccessor = new VNPostalCode(postalCodeSuccessor);
        ...
        {
            Assert.IsNull((new VNPostalCode(postalCode)).Predecessor);
        }

        [TestCase("999999")]
        public void Successor_LastInRange_ReturnsNull(string postalCode)
        {
            Assert.IsNull((new VNPostalCode(postalCode)).Successor);
        }
        ...
        {
            Assert.Throws<ArgumentException>(() => new VNPostalCode(postalCode));
        }

        public void CompareTo_ReturnsExpectedSign(string postalCodeBefore, string postalCodeAfter)
        {
            var b = new VNPostalCode(postalCodeBefore);
            var a = new VNPostalCode(postalCodeAfter);
        ...
        {
            var x = (new VNPostalCode(code)).Predecessor;
            bool result = true;
        ...
        {
            var x = (new VNPostalCode(code)).Predecessor;
            Assert.IsTrue(x.GetType() == typeof(VNPostalCode));
        }
        ...
        {
            var x = (new VNPostalCode(code)).Successor;
            Assert.IsTrue(x.GetType() == typeof(VNPostalCode));
        }
        ...
        {
            var x = (new VNPostalCode(code)).ExpandPostalCodeAsHighestInRange();
            Assert.IsTrue(x.GetType() == typeof(VNPostalCode));
        }
        ...
        {
            var x = (new VNPostalCode(code)).ExpandPostalCodeAsLowestInRange();
            Assert.IsTrue(x.GetType() == typeof(VNPostalCode));
        }
        ...
        {
            var x = new VNPostalCode(code);
            var y = new VNPostalCode(code);
        ...
        {
            var country = CountryFactory.Instance.CreateCountry("VN");
