...
{
    [TestFixture]
    internal class BEPostalCodeTests
    {
        ...
        {
            var code = new BEPostalCode(postalCode);
            var codePredecessor = new BEPostalCode(postalCodePredecessor);
        ...
        {
            var code = new BEPostalCode(postalCode);
            var codeSuccessor = new BEPostalCode(postalCodeSuccessor);
        ...
        {
            Assert.IsNull((new BEPostalCode(postalCode)).Predecessor);
        }

        [TestCase("9999")]
        public void Successor_LastInRange_ReturnsNull(string postalCode)
        {
            Assert.IsNull((new BEPostalCode(postalCode)).Successor);
        }
        ...
        {
            var x = (new BEPostalCode(code)).Predecessor;
            bool result = true;
        ...
        {
            var x = (new BEPostalCode(code)).Successor;
            Assert.IsTrue(x.GetType() == typeof(BEPostalCode));
        }
        ...
        {
            var x = (new BEPostalCode(code)).ExpandPostalCodeAsHighestInRange();
            Assert.IsTrue(x.GetType() == typeof(BEPostalCode));
        }
        ...
        {
            var x = (new BEPostalCode(code)).ExpandPostalCodeAsLowestInRange();
            Assert.IsTrue(x.GetType() == typeof(BEPostalCode));
        }
        ...
        {
            var x = new BEPostalCode(code);
            var y = new BEPostalCode(code);
        ...
        {
            var country = CountryFactory.Instance.CreateCountry("BE");
