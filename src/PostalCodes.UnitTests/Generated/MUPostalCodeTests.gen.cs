...
{
    [TestFixture]
    internal class MUPostalCodeTests
    {
        ...
        {
            var code = new MUPostalCode(postalCode);
            var codePredecessor = new MUPostalCode(postalCodePredecessor);
        ...
        {
            var code = new MUPostalCode(postalCode);
            var codeSuccessor = new MUPostalCode(postalCodeSuccessor);
        ...
        {
            Assert.IsNull((new MUPostalCode(postalCode)).Predecessor);
        }

        [TestCase("99999")]
        public void Successor_LastInRange_ReturnsNull(string postalCode)
        {
            Assert.IsNull((new MUPostalCode(postalCode)).Successor);
        }
        ...
        {
            Assert.Throws<ArgumentException>(() => new MUPostalCode(postalCode));
        }

        public void CompareTo_ReturnsExpectedSign(string postalCodeBefore, string postalCodeAfter)
        {
            var b = new MUPostalCode(postalCodeBefore);
            var a = new MUPostalCode(postalCodeAfter);
        ...
        {
            var x = (new MUPostalCode(code)).Predecessor;
            bool result = true;
        ...
        {
            var x = (new MUPostalCode(code)).Predecessor;
            Assert.IsTrue(x.GetType() == typeof(MUPostalCode));
        }
        ...
        {
            var x = (new MUPostalCode(code)).Successor;
            Assert.IsTrue(x.GetType() == typeof(MUPostalCode));
        }
        ...
        {
            var x = (new MUPostalCode(code)).ExpandPostalCodeAsHighestInRange();
            Assert.IsTrue(x.GetType() == typeof(MUPostalCode));
        }
        ...
        {
            var x = (new MUPostalCode(code)).ExpandPostalCodeAsLowestInRange();
            Assert.IsTrue(x.GetType() == typeof(MUPostalCode));
        }
        ...
        {
            var x = new MUPostalCode(code);
            var y = new MUPostalCode(code);
        ...
        {
            var country = CountryFactory.Instance.CreateCountry("MU");
